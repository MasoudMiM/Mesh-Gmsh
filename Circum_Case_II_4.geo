...
CrackDepth = 3/6;
DmgAngle = Pi/2; // Should be less than Pi/2 for this code!
CrackLength = 0.1;
...
surfaceVector_bd_V2[] = Extrude {{0, 0, 1}, {0, 0, 0}, Pi/4} {
  Surface{surfaceVector_bd_V1[0]}; Layers{ ( (Pi/4)/(Pi/2)  ) *NlayersThickness};Recombine;
};


surfaceVector_bd_V3[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2-Pi/4} {
  Surface{surfaceVector_bd_V2[0]}; Layers{ ((Pi/2-Pi/4)/(Pi/2)) * NlayersThickness};Recombine;
};
...
surfaceVector_bd_V6[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2} {
  Surface{surfaceVector_bd_V5[0]}; Layers{NlayersThickness};Recombine;
};
...
surfaceVector_d_V3[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2-Pi/4} {
  Surface{surfaceVector_d_V30[4]}; Layers{( (Pi/2-Pi/4)/(Pi/2) )*NlayersThickness};Recombine;
};

Printf("surfaceVector_d_V30[4] is %f", surfaceVector_d_V30[4]);

Point(300) = {-R_in*Cos(Pi/4), R_in*Sin(Pi/4), CrackLoc, PointCoefMesh};
Point(400) = {-R_out*Cos(Pi/4), R_out*Sin(Pi/4), CrackLoc, PointCoefMesh};
Line(300) = {300,400};
...
Printf("surfaceVector_L_3[1] is %f", surfaceVector_L_3[1]);

surfaceVector_d_V4[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2-Pi/4} {
  Surface{surfaceVector_L_3[1]}; Layers{( (Pi/2-Pi/4)/(Pi/2) )*NlayersThickness};Recombine;
};
...
surfaceVector_ad_V2[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/4} {
  Surface{surfaceVector_ad_V1[0]}; Layers{( (Pi/4)/(Pi/2) )*NlayersThickness};Recombine;
};

surfaceVector_ad_V3[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2-Pi/4} {
  Surface{surfaceVector_ad_V2[0]}; Layers{( (Pi/2-Pi/4)/(Pi/2) )*NlayersThickness};Recombine;
};
