...
DmgAngle = Pi/2; 
DmgAngle2 = Pi/4; // Should be less than Pi/2 for this code!
np = 4; // number of elements for crack's width
...
//######################################################################################################
//----------------------------------------- Section of the pipe with defect  (First Pi/2)
//----------------- Damage Itself (start)
...
//######################################################################################################
//----------------------------------------- Section of the pipe with defect  (Second Pi/2)
//----------------- Damage Itself (start)
//-------- Thin layer of the crack section
Point(300000) = {0, R_in, CrackLoc+CrackLength, PointCoefMesh};
Point(400000) = {0, R_outCr, CrackLoc+CrackLength, PointCoefMesh};
Line(300000) = {300000,400000};

surfaceVector_L_3000[]=Extrude {{0,0,1},{0,0,CrackLoc+CrackLength},DmgAngle2}{
        Line{300000}; Layers{(DmgAngle2/(Pi/2))*NlayersThickness};Recombine;
};

surfaceVector_d_V3000[]=Extrude {0,0,CrackLength} {
  Surface{300004}; Layers{NumberOfLengthElm}; Recombine;
};

//-------- The rest of the Pi/2 section
Point(30000) = {-R_in*Cos(DmgAngle2), R_in*Sin(DmgAngle2), CrackLoc+CrackLength, PointCoefMesh};
Point(40000) = {-R_out*Cos(DmgAngle2), R_out*Sin(DmgAngle2), CrackLoc+CrackLength, PointCoefMesh};
Line(30000) = {30000,40000};

surfaceVector_L_3000[]=Extrude {0,0,CrackLength}{
        Line{30000}; Layers{NumberOfLengthElm};Recombine;
};

surfaceVector_d_V300[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2-DmgAngle2} {
  Surface{surfaceVector_L_3000[1]}; Layers{( (Pi/2-DmgAngle2)/(Pi/2) )*NlayersThickness};Recombine;
};
//----------------- Damage Itself (End)

surfaceVector_d_V400[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2} {
  Surface{surfaceVector_d_V300[0]}; Layers{NlayersThickness};Recombine;
};

surfaceVector_d_V500[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2} {
  Surface{surfaceVector_d_V400[0]}; Layers{NlayersThickness};Recombine;
};

surfaceVector_d_V600[]=Extrude {{0, 0, 1}, {0, 0, 0}, Pi/2} {
  Surface{surfaceVector_d_V500[0]}; Layers{NlayersThickness};Recombine;
};
...
jj = 0;
th(0) = 0;
For jj In {1:(DmgAngle2/(Pi/2) )*NlayersThickness:1.0}
  th(jj) = DmgAngle2/(  (DmgAngle2/(Pi/2) )  *NlayersThickness)*jj;
  th(jj) = DmgAngle2+th(jj);
  Point(70000+20*jj) = {-R_outCr*Cos(th(jj-1)), R_outCr*Sin(th(jj-1)), CrackLoc+CrackLength , PointCoefMesh};
  Point(80000+50*jj) = {-R_out*Cos(th(jj-1)), R_out*Sin(th(jj-1)), CrackLoc+CrackLength, PointCoefMesh};

  Line(80000+100*jj) = {70000+20*jj,80000+50*jj};

  Zcoord = CrackLoc+2*CrackLength - jj * ( (CrackLength-ZGap)/( (DmgAngle2/(Pi/2)) *NlayersThickness) );
  ZcoordNew = Zcoord - (CrackLoc+CrackLength) - ZGap;
  TempVal = Fmod ( Fabs(ZcoordNew), CrackLength/NumberOfLengthElm );
  ...
  surfaceVector_L_30000[]=Extrude {{0,0,1},{0, 0, CrackLoc+CrackLength},th(jj)-th(jj-1)}{
        Line{80000+100*jj}; Layers{1};Recombine;
  };
  
  If (ZcoordNew>=ZGap)
    nlayer = Ceil(Fabs(ZcoordNew/CrackLength)*NumberOfLengthElm); 
    surfaceVector_d_V30000[]=Extrude {0,0,ZcoordNew} {
      Surface{surfaceVector_L_30000[1]}; Layers{nlayer};Recombine;
    };
    Printf("For jj=%g, number of elements in axial direction is %g", jj, nlayer );
  EndIf
